...
_COMPONENT_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ...
    (
        "district",
        re.compile(
            _NB + r"([А-ЯЁа-яё\-]+(?:ий|ый|ой|ский|ской|ого))\s+"
            r"(?:р-н|район\w*|г\.\s?о\.|городск\w+\s+округ\w*)",
            FLAGS,
        ),
    ),
    (
        "city",
        re.compile(
            _NB + r"(?:г\.(?!р\.|рожд|\s?о\.)|гор\.|город\w*(?!\s+(?:проживани|рождени|регистраци|пребывани))"
            r"|г(?=\s)|пгт\.?|пос\.|посёлок|поселок|с\.|село|дер\.|деревн\w+|д\.(?=\s*(?-i:[А-ЯЁ])[а-яё])"
            r"|ст-ца|станиц\w+)"
            r"\s*(?:им\.\s*|имени\s+)?([А-ЯЁа-яё][А-ЯЁа-яё\-]*(?:\s+(?-i:[А-ЯЁ])[а-яё\-]+)?)",
            FLAGS,
        ),
    ),
    (
        ...
        re.compile(
            ...
            r"|алле[яи]|линия(?=\s+(?-i:[А-ЯЁ0-9]))|тракт)\s*"
    ...
    (
        ...
        re.compile(
            ...
            r"(?:улица|ул\.|проспект|пр-кт|пр-т|просп\.|пер\.|переулок|шоссе|бульвар|набережная|площадь)",
            FLAGS,
        ),
    ),
    (
