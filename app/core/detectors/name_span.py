...
Детекторы находят ядро ФИО («Пётр Алексеевич», «Сидоров П»), а соседние части остаются
открытыми: «С. Пётр Алексеевич», «Пётр Сидоров Алексеевич», «Пётр Алексеевич С.». Функция
...
# Инициал слева: «С. Пётр», «С.Пётр» (перед буквой инициала — не буква).
_LEFT_INITIAL_RE = re.compile(r"(?<![A-Za-zА-ЯЁа-яё.])([A-Za-zА-ЯЁа-яё])\.\s?$")
# Инициалы справа: « П.», « П. А.», « П.А.» (после точки не должна идти буква того же слова).
...
# Ядро заканчивается одиночной буквой-инициалом без точки: «Сидоров П» + «.».
