...
_DEPT_CODE_RE = re.compile(
    r"(?:код\w*\s+подразделени\w*|к/п|к\.\s?п\.)\s*[:№\-–]?\s*(\d{3}[\s\-]?\d{3})(?!\d)", FLAGS
)
...
_COUNTRIES = (
    ...
    r"|Казахстан\w*|казахстанск\w+|Узбекистан\w*|узбекск\w+|Таджикистан\w*|таджикск\w+"
    r"|Кыргызстан\w*|Кыргызской\s+Республики|Киргизи[ия]|киргизск\w+|Армени[ияю]|армянск\w+"
    r"|Азербайджан\w*|азербайджанск\w+|Грузи[ия]|грузинск\w+|Молдов[аыу]|Молдави[ия]|Туркменистан\w*"
...
_BIRTH_PLACE_RE = re.compile(
    ...
    r"([А-ЯЁа-яё][А-ЯЁа-яё\-]*(?:\s+[А-ЯЁа-яё][А-ЯЁа-яё\-]*){0,2})",
    FLAGS,
)
