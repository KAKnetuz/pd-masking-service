...
* между меткой и значением допускаются служебные слова («ПИН второй карты: 5678»,
  «в поле CVV указан код 123», «ИНН клиента 771234567890»);
* метка может стоять после значения («код 258 неверно введен CVV»);
* регистр не важен.

Покрывает: ПИН, CVV, ИНН (в т.ч. группами цифр), код подразделения, орган выдачи паспорта,
ФИО после меток роли человека («Получатель платежа: Иванов И.»), ФИО и номера, записанные
как всё сообщение («Ли Вэй», «12-34-567890»).
"""
...
# Слитное ФИО: «ИвановИванПетрович», «ИВАНОВИВАНПЕТРОВИЧ» (оканчивается отчеством).
...
# Латинские имя и фамилия в начале русской фразы: «Sidorova Anna подтвердила …».
